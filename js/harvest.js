...
async function htLoad(){
  const _hideHarvestOverlay = () => { const _ov=document.getElementById('harvest-loading-overlay'); if(_ov) _ov.style.display='none'; };
...
async function htShowCollectorPopup(collectorName){
  if(!window._htColByCol || !Object.keys(window._htColByCol).length) await loadTodaySummary();
